...
class TestScopedHistory:
    ...
    def test_list_threads_combines_active_and_archived(self, mock_req, monkeypatch):
        ...
        def response(method, path, token, **kwargs):
            if path == "/channels/20":
                return {"id": "20", "type": 0, "parent_id": "10", "guild_id": "1"}
            if path == "/guilds/1/threads/active":
                return {"threads": [{
                    "id": "30", "name": "active", "type": 11, "parent_id": "20",
                    "thread_metadata": {"archived": False},
                }]}
            if path == "/channels/20/threads/archived/public":
                return {"threads": [{
                    "id": "31", "name": "archived", "type": 11, "parent_id": "20",
                    "thread_metadata": {"archived": True, "archive_timestamp": "2026-01-01T00:00:00Z"},
                }], "has_more": False}
            raise AssertionError(path)

        mock_req.side_effect = response
        result = json.loads(discord_core(action="list_threads", channel_id="20"))
        assert [thread["name"] for thread in result["threads"]] == ["active", "archived"]
        assert result["archived_has_more"] is False
